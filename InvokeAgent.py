...
theRegion = st.secrets["AWS_REGION"]
agentId = st.secrets["AGENT_ID"]
agentAliasId = st.secrets["AGENT_ALIAS_ID"]
...
def sigv4_request(
    ...
    if not credentials:
        # Crear un objeto de credenciales usando los secretos
        credentials = Session(aws_access_key_id=st.secrets["aws_access_key_id"],
                              aws_secret_access_key=st.secrets["aws_secret_access_key"],
                              aws_session_token=st.secrets.get("aws_session_token")  # Este puede ser None si no usas tokens de sesión
